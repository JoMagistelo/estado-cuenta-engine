...
def _process_prepared_statement(
    ...
    if document is None:
        raise RuntimeError(
            f"El documento no contiene un DocumentData válido para el método "
            f"'{prepared.processing_method}'."
        )
        ...
        while future_map:
            if _cancel_requested(cancel_event):
                ...
                for future, data in list(future_map.items()):
                    _future_type, index, file_name, prepared = data
                    future.cancel()
