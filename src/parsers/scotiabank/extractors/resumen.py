...
class SummaryTableGeometry:
    """Geometría de la tabla ``Resumen de Saldos`` de la página 1."""
...
def _summary_geometry(
    words: Sequence[SpatialWord],
    lines_page_one: Sequence[SpatialLine],
) -> SummaryTableGeometry:
    width = _document_width(words)
    title = _summary_title_line(lines_page_one, width)
...
def _annual_rate(
    ...
    # Algunos layouts imprimen la tasa sólo en la tabla de sobregiro de la
    # misma página 1. Se conserva como fuente textual explícita; no se deriva.
    for line in lines:
...
def _build_summary_values(words: List[SpatialWord]) -> SummaryValues:
    """Extrae el resumen exclusivamente desde la página 1.

    Los movimientos no participan como fuente alternativa y ningún saldo o
    total se reconstruye mediante ecuaciones. Si el OCR omite un importe de la
    tabla, el campo queda en su valor por defecto para que la validación revele
    la ausencia en lugar de introducir un valor estimado.
    """

    lines_page_one = _page_lines(words, 1)
    geometry = _summary_geometry(words, lines_page_one)
    ...
    return SummaryValues(
        ...
        tasa_bruta_anual=_annual_rate(lines_page_one, geometry),
...
def extract_resumen_financiero_words(
    ...
    La fuente de verdad es la tabla ``Resumen de Saldos`` de la página 1.
    No se suman cargos/abonos de movimientos y no se recalculan saldos para
    completar datos que el motor de lectura no haya entregado.
    """
