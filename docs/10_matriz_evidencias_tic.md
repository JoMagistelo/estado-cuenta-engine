...
| Datos personales en Git | Implementado a nivel repo | `.gitignore`, pruebas opt-in, `SECURITY.md`, checklist PR | supervisión y procedimiento institucional |
...
| Gestión de incidentes | Parcial | `SECURITY.md` y runbook del repositorio | canal, responsables, CSIRT/TIC, tiempos y formatos institucionales |
...
| Gestión de parches | Parcial | Dependabot para Python/Actions | Windows Server, Tesseract y demás componentes de infraestructura |
...
El workflow `Production readiness` debe demostrar en cada PR:
