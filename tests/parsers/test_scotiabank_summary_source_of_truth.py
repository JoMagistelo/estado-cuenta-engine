...
def _label_row(
    ...
    *,
    ...
    for token in tokens:
        ...
        words.append(_word(token, x, x + width, top, page=page))
        x += width + 2.2

    if amount is not None:
        words.append(_word(amount, 198.0, 238.0, top, page=page))
...
def _summary_words(
    *,
    ...
    words: list[dict[str, object]] = [
        _word("Periodo", 424.0, 449.0, 112.0, page=page),
        _word("14-NOV-23/13-DIC-23", 469.0, 542.0, 112.0, page=page),
        _word("Resumen", 118.0, 150.0, 266.0, page=page),
        _word("de", 153.0, 161.0, 266.0, page=page),
        _word("Saldos", 164.0, 188.0, 266.0, page=page),
    ]
    ...
    for top, tokens, amount in rows:
        words.extend(_label_row(top, tokens, amount, page=page))
    ...
    words.extend(
        [
            _word("$888,888.88", 430.0, 490.0, 300.0, page=page),
            _word("$777,777.77", 470.0, 530.0, 336.0, page=page),
            _word("$666,666.66", 480.0, 540.0, 390.0, page=page),
        ]
    )
...
def _preliminary_pages() -> list[dict[str, object]]:
    return [
        # Publicidad: contiene palabras que podrían parecer relevantes, pero no
        # la estructura suficiente para convertirse en página del resumen.
        _word("Resumen", 118.0, 150.0, 180.0, page=1),
        _word("de", 153.0, 161.0, 180.0, page=1),
        _word("Saldos", 164.0, 188.0, 180.0, page=1),
        _word("Saldo", 68.0, 90.0, 200.0, page=1),
        _word("inicial", 93.0, 121.0, 200.0, page=1),
        _word("$999,999.99", 198.0, 238.0, 200.0, page=1),
        _word("PROMOCION", 250.0, 320.0, 220.0, page=1),
        # Segunda hoja preliminar de seguridad/avisos.
        _word("Protege", 80.0, 120.0, 160.0, page=2),
        _word("tu", 124.0, 136.0, 160.0, page=2),
        _word("cuenta", 140.0, 180.0, 160.0, page=2),
        _word("Depositos", 80.0, 130.0, 200.0, page=2),
        _word("seguros", 134.0, 175.0, 200.0, page=2),
    ]


def test_summary_uses_printed_table_values_not_movement_totals(
...
def test_preliminary_pages_do_not_change_summary_source() -> None:
    words = [*_preliminary_pages(), *_summary_words(page=3)]

    context = scotia_summary._summary_page_context(words)
